...
#define SDMEM_INPUT_CLOCK  32000000
...
#define I_O_BLOCK_SIZE_ARG_1 (0x80022202)
...
#define LSB_ADDR_M4            (0x8001F500)
#define MSB_ADDR_M4           (0x8001F746)  //GPIO_4613
#define LSB_ADDR_PAD            (0x8001F530)
#define MSB_ADDR_PAD            (0x8001F741)  //GPIO_4613
#define LSB_ADDR_REN            (0x8001F500)
#define MSB_ADDR_REN            (0x8001F746)  //GPIO_4613
#define LSB_ADDR_DIR            (0x8001F513)
#define MSB_ADDR_DIR            (0x8001F746)  //GPIO_4613
#define LSB_ADDR_PIN            (0x8001F513)
#define MSB_ADDR_PIN            (0x8001F746)  //GPIO_4613
#define SD_CMD0             (0)
#define SD_CMD1             (1)
#define SD_CMD2             (2)
#define SD_CMD3             (3)
...
typedef struct _stc_sdcmd_ocr
{
...
#endif
    uint32_t u32Reserved1   :8;    
    uint32_t u32V20_21      :1;
    uint32_t u32V21_22      :1;
    uint32_t u32V22_23      :1;
    uint32_t u32V23_24      :1;
    uint32_t u32V24_25      :1;
    uint32_t u32V25_26      :1;
    uint32_t u32V26_27      :1;
    ...
    uint32_t u32MEM_PRSNT   :1;
    uint32_t u32I_O_NUM     :3;
    uint32_t u32CARD_READY  :1;
}stc_sdcmd_ocr_t;
/**
 ******************************************************************************
...
typedef struct _stc_sdcard_config
{
    ...
    uint16_t                Byte_Block_Count;
    uint8_t                 Func_Num   : 3; 
    uint8_t                 Block_Mode : 1;
    uint8_t                 Op_Code    : 1;
}stc_sdcard_config_t;

typedef struct SDIO_CCCR_REG_Info{
    __IO uint8_t  CCID_SDIO_REV_REG;                 /*!< CCID SDIO revision version Register                                           */
    __IO uint8_t  SD_SPEC_REV_REG;                 /*!< CCID SDIO revision version Register                                           */
    __IO uint8_t  I_O_ENABLE_REG;                 /*!< CCID SDIO revision version Register                                           */
    __IO uint8_t  I_O_READY;
    __IO uint8_t  INT_ENABLE;
    __IO uint8_t  INT_PENDING;
    __IO uint8_t  I_O_ABORT;
    __IO uint8_t  BUS_CONTRO;
    __IO uint8_t  CARD_CAPICITY;
    __IO uint8_t  COMMON_CIS_POINTER0;
    __IO uint8_t  COMMON_CIS_POINTER1;
    __IO uint8_t  COMMON_CIS_POINTER2;
    __IO uint8_t  BUS_SPEND;
    __IO uint8_t  FUNC_SELECT;
    __IO uint8_t  EXEC_FLAGS;
    __IO uint8_t  READY_FLAGS;
    __IO uint16_t FUCN0_BLOCK_SIZE;
    __IO uint8_t  POWER_CONTROL;
    __IO uint8_t  BUS_SPEED_SELECT;
    __IO uint8_t  UHS_I_SUPPORT;
    __IO uint8_t  DRIVER_STRENGTH;
    __IO uint8_t  INTR_EXTENSION;
}__attribute__((__packed__)) SDIO_CCCR_REG_Info_t;
